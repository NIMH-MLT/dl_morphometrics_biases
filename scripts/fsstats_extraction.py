...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "pipeline_roots", help="Comma‑separated pipeline root dirs (first = recon‑all)"
    )
    p.add_argument(
        "--metrics", default=",".join(METRIC_MAPPING.keys()), help="Aparc metrics"
    )
    p.add_argument("--atlases", default=",.a2009s,.DKTatlas", help="Atlas suffix list")
    p.add_argument(
        "--brain-metrics",
        default=DEFAULT_BRAIN_METRICS,
        help="Brainvol metrics to keep",
    )
    p.add_argument(
        "--invalidate-cache", action="store_true", help="Recompute cache regardless"
    )
...
def aparc_filename(hemi: str, atlas_suffix: str) -> str:
    return (
        f"{hemi}.aparc{atlas_suffix}.stats" if atlas_suffix else f"{hemi}.aparc.stats"
    )
...
def main() -> None:
    ...
    aparc_metrics = args.metrics.split(",")
    ...
    out_parent = recon_root.parents[1] / OUT_SUBDIR
    ...
    if cache_path.exists() and not args.invalidate_cache:
        ...
        valid_sessions = {
            tuple(k): {Path(pk): Path(sd) for pk, sd in v.items()}
            for k, v in cache_obj["sessions"].items()
        }
        ...
        pickle.dump(
            {
                "sessions": {
                    k: {str(pk): str(sd) for pk, sd in v.items()}
                    for k, v in valid_sessions.items()
                }
            },
            cache_path.open("wb"),
        )
    ...
    aparc_rows = {
        pr: {a: {m: [] for m in aparc_metrics} for a in atlases} for pr in pipelines
    }
    ...
    for (sub_id, ses_id), pr_map in valid_sessions.items():
        for pr, sd in pr_map.items():
            ...
            if bv_df is not None:
                ...
                pivot = (
                    bv_df.pivot(
                        index=["subject", "session"], columns="Measure", values="Value"
                    ).reset_index()
            ...
            for atlas_key in atlases:
                lh = fs.read(
                    str(sd / aparc_filename("lh", atlas_key))
                ).structural_measurements
                rh = fs.read(
                    str(sd / aparc_filename("rh", atlas_key))
                ).structural_measurements
                lh_p = (
                    lh.assign(structure_name=lambda d: d["structure_name"] + "_lh")
                    .set_index("structure_name")[aparc_metrics]
                    .T
                )
                rh_p = (
                    rh.assign(structure_name=lambda d: d["structure_name"] + "_rh")
                    .set_index("structure_name")[aparc_metrics]
                    .T
                )
    ...
    # write outputs
    for pr in pipelines:
        out_dir = out_parent / pr.name
        out_dir.mkdir(parents=True, exist_ok=True)
        # brainconfounds
        if brain_rows[pr]:
            pd.concat(brain_rows[pr], ignore_index=True).to_csv(
                out_dir / "BrainConfounds.tsv", sep="\t", index=False
            )
            # aparc TSVs
        for atlas_key, metric_map in aparc_rows[pr].items():
            ...
            for metric, rows in metric_map.items():
                ...
                ordered_cols = [
                    "subject",
                    "session",
                    *[c for c in df.columns if c not in {"subject", "session"}],
                ]
                df = df[ordered_cols]
                df.to_csv(
                    out_dir
                    / f"atlas-{atlas_name}_{METRIC_MAPPING.get(metric, metric)}.tsv",
                    sep="	",
                    index=False,
                )
